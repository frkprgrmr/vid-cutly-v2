    ...
    <article className={`clip-card ${active ? "active" : ""}`}>
      ...
      <div className="clip-main">
        ...
        {editing && <div className="editor-grid">
          ...
          <label className="wide">Framing video<select value={form.framing_mode} onChange={(e) => setForm({ ...form, framing_mode: e.target.value as Clip["framing_mode"] })}><option value="auto">Auto — ikuti wajah / pilih layout</option><option value="left">Kiri — kunci pembicara kiri</option><option value="center">Tengah — kunci bagian tengah</option><option value="right">Kanan — kunci pembicara kanan</option><option value="fit_blur">Fit + Blur — tampilkan frame lengkap</option><option value="split">Split-screen — kiri di atas, kanan di bawah</option></select><small>{framingHelp(form.framing_mode)}</small></label>
        </div>}
...
function framingHelp(mode: Clip["framing_mode"]) { return ({ auto: "Cocok untuk mayoritas clip; sistem memilih crop wajah, split-screen, atau Fit + Blur.", left: "Gunakan jika pembicara utama selalu berada di sisi kiri video sumber.", center: "Gunakan untuk objek atau pembicara yang memang berada di tengah.", right: "Gunakan jika pembicara utama selalu berada di sisi kanan video sumber.", fit_blur: "Paling aman ketika wajah tidak terdeteksi atau semua orang perlu terlihat.", split: "Cocok untuk dialog dua orang yang duduk berjauhan." })[mode]; }
